#!/usr/bin/env python3
"""
classify_openai.py — Classify crawled YouTube videos using an OpenAI model.

Run locally:
    pip install openai tqdm
    python classify_openai.py
...
Output schema (one JSON object per line in OUTPUT_PATH):
{
    "video_id": str,
    "title": str,
    "openai_label": str | null,
    "openai_error": str | null
}
"""
...
# Delay between requests. Nano/mini models usually allow high RPM — start low
# and the exponential backoff handles any 429s automatically.
...
def build_user_message(video: dict) -> str:
...
def classify_with_backoff(client: OpenAI, video: dict, max_retries: int = 6) -> str:
    user_msg = build_user_message(video)
    delay = 30  # initial backoff on rate-limit
...
def main():
    if not OPENAI_API_KEY:
        raise SystemExit("Paste your OpenAI API key into OPENAI_API_KEY at the top of this file.")
    ...
    with open(OUTPUT_PATH, "a", encoding="utf-8") as out_f:
        try:
            for video_id, video in tqdm(remaining.items(), total=len(remaining)):
                ...
                try:
                    result["openai_label"] = classify_with_backoff(client, video)
                    result["openai_error"] = None
                except Exception as e:
                    result["openai_label"] = None
