...
def get_first_recommendation_url(page) -> str | None:
    """
    Extract the URL of the first recommendation on the watch page sidebar.
    Does NOT click it — we navigate directly to avoid stale element issues.
    """
    ...
    # The sidebar recommendations are <a> tags with /watch hrefs inside
    # the secondary results panel
    ...
    for sel in selectors:
        ...
        for j in range(min(count, 10)):
            ...
            if href and "/watch" in href and "/shorts/" not in href:
...
def crawl(
    ...
    with sync_playwright() as p:
        ...
        for i in range(n_iterations):
            ...
            if i < n_iterations - 1:
                next_url = get_first_recommendation_url(page)
                if not next_url:
